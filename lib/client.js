window.__ModuleLoader__.load({ id: "dsh-ops-console", factory: (require) => {
  ...
  const I18N = {
    zh: {
      ...
      "server.restartHint": "安全重启会先隔离预检；失败时不会停服，启动失败会自动恢复或进入安全模式。", "server.restart": "安全重启",
      "server.confirm": "确认重启？", "server.confirmBtn": "确认重启", "server.cancel": "取消",
      "server.restarting": "重启中…", "server.logs": "最近日志", "server.liveTail": "实时跟尾",
      "server.noLogs": "（暂无日志）",
      "guardian.title": "启动防护", "guardian.mode": "当前模式", "guardian.production": "正式模式",
      "guardian.safe": "安全模式", "guardian.recovered": "已自动恢复", "guardian.unknown": "未知",
      "guardian.lkg": "黄金版本", "guardian.ready": "已建立", "guardian.missing": "尚未建立",
      "guardian.backups": "部署备份", "guardian.preflight": "运行完整预检", "guardian.preflighting": "预检中…",
      "guardian.pass": "预检通过", "guardian.fail": "预检失败", "guardian.recover": "恢复黄金版本",
      "guardian.recoverConfirm": "确认恢复黄金版本并安全重启？", "guardian.unavailable": "外部 guardian 不可用",
      ...
      "settings.token": "远程运维令牌", "settings.tokenHint": "仅在本机页面自动读取；远程设备请从本机复制后粘贴。", "settings.tokenSave": "保存令牌",
    ...
    en: {
      ...
      "server.restartHint": "Safe restart runs an isolated preflight first; failed boots recover automatically or enter safe mode.", "server.restart": "Safe restart",
      ...
      "guardian.title": "Startup protection", "guardian.mode": "Current mode", "guardian.production": "Production",
      "guardian.safe": "Safe mode", "guardian.recovered": "Auto-recovered", "guardian.unknown": "Unknown",
      "guardian.lkg": "Golden snapshot", "guardian.ready": "Ready", "guardian.missing": "Missing",
      "guardian.backups": "Deployment backups", "guardian.preflight": "Run full preflight", "guardian.preflighting": "Checking…",
      "guardian.pass": "Preflight passed", "guardian.fail": "Preflight failed", "guardian.recover": "Restore golden snapshot",
      "guardian.recoverConfirm": "Restore the golden snapshot and safely restart?", "guardian.unavailable": "External guardian unavailable",
      ...
      "settings.token": "Remote admin token", "settings.tokenHint": "Read automatically on the local page; paste a token copied from the local page on remote devices.", "settings.tokenSave": "Save token",
  ...
  function post(path) {
    return api(path, { method: "POST", headers: { "content-type": "application/json", "x-dsh-ops-token": adminToken() } });
  }

  function postJson(path, body) {
    return api(path, { method: "POST", headers: { "content-type": "application/json", "x-dsh-ops-token": adminToken() }, body: JSON.stringify(body) });
  }
  ...
  function ServerTab() {
    ...
    const guardian = useApi("/dsh-ops/guardian");
    const settings = useApi("/dsh-ops/settings");
    ...
    useInterval(guardian.reload, 1000, restarting || guardBusy);
    ...
    const doRestart = () => {
      ...
      post("/dsh-ops/restart")
        .then((result) => { if (!result.ok) setGuardMsg(result); guardian.reload(); })
        .catch((error) => setGuardMsg({ ok: false, error: String(error) }))
        .finally(() => {});
    };

    const s = status.data;
    const g = guardian.data;
    const operation = g?.operation;
    useEffect(() => {
      if (operation?.phase === "completed" || operation?.phase === "failed") {
        setRestarting(false);
        setGuardBusy(false);
        if (operation.phase === "failed") setGuardMsg({ ok: false, error: operation.message });
      }
    }, [operation?.updatedAt]);
    const guardianReady = g?.available === true && g?.compatible !== false;
    const guardMode = g?.state?.mode || "unknown";
    const guardModeLabel = t("guardian." + (guardMode === "production" || guardMode === "safe" || guardMode === "recovered" ? guardMode : "unknown"));
    const runPreflight = () => {
      setGuardBusy(true);
      setGuardMsg(null);
      post("/dsh-ops/preflight")
        .then((result) => setGuardMsg(result))
        .catch((error) => setGuardMsg({ ok: false, error: String(error) }))
        .finally(() => { setGuardBusy(false); guardian.reload(); });
    };
    const recover = () => {
      if (!window.confirm(t("guardian.recoverConfirm"))) return;
      setGuardBusy(true);
      post("/dsh-ops/recover")
        .then((result) => setGuardMsg(result))
        .catch((error) => setGuardMsg({ ok: false, error: String(error) }))
        .finally(() => guardian.reload());
    };
      ...
      el(Card, {
        ...
        !confirm
          ...
          : el("div", { className: "dshops-row" },
              ...
              el(Btn, { danger: true, disabled: restarting || !guardianReady, onClick: doRestart }, restarting ? t("server.restarting") : t("server.confirmBtn")),
              el(Btn, { onClick: () => setConfirm(false) }, t("server.cancel")),
            ),
      ),
      el(Card, {
        title: t("guardian.title"),
        right: !guardianReady ? el(Badge, { tone: "bad" }, t("guardian.unavailable"))
          : el(Badge, { tone: guardMode === "safe" ? "warn" : "ok" }, guardModeLabel),
      },
        guardian.loading && !g ? el("div", { className: "dshops-muted" }, t("common.loading")) : null,
        Error({ children: guardian.error }),
        g ? el(React.Fragment, null,
          el("div", { className: "dshops-row" }, el("span", { className: "dshops-muted" }, t("guardian.mode")), el("span", null, guardModeLabel)),
          el("div", { className: "dshops-row" }, el("span", { className: "dshops-muted" }, "Guardian"), el("span", null,
            g.guardianVersion ? ("v" + g.guardianVersion + " · protocol " + (g.protocolVersion || 0)) : "—")),
          el("div", { className: "dshops-row" }, el("span", { className: "dshops-muted" }, t("guardian.lkg")),
            el(Badge, { tone: g.lastKnownGood ? "ok" : "bad" }, g.lastKnownGood ? t("guardian.ready") : t("guardian.missing"))),
          el("div", { className: "dshops-row" }, el("span", { className: "dshops-muted" }, t("guardian.backups")), el("span", null, String(g.deploymentBackups?.length || 0))),
          operation ? el("div", { className: "dshops-row" },
            el("span", { className: "dshops-muted" }, "操作进度"),
            el("span", null, (operation.message || operation.command || "处理中") + (operation.percent != null ? "（" + operation.percent + "%）" : "")),
          ) : null,
          el("div", { className: "dshops-row" },
            el(Btn, { primary: true, disabled: guardBusy || !guardianReady, onClick: runPreflight }, guardBusy ? t("guardian.preflighting") : t("guardian.preflight")),
            el(Btn, { danger: true, disabled: guardBusy || !guardianReady || !g.lastKnownGood, onClick: recover }, t("guardian.recover")),
          ),
          guardMsg ? el("div", { className: guardMsg.ok ? "dshops-muted" : "dshops-error" },
            guardMsg.ok ? t("guardian.pass") : t("guardian.fail") + ": " + (guardMsg.error || guardMsg.issues?.join("; ") || "")) : null,
        ) : null,
      ),
      el(Card, {
        title: t("server.logs"),
  ...
  function SettingsTab() {
    ...
    useEffect(() => {
      if (typeof auth.data?.token === "string" && auth.data.token.length >= 32) setToken(auth.data.token);
    }, [auth.data?.token]);
    ...
    const save = () => {
      ...
      setBusy(true);
      setSaved(null);
      postJson("/dsh-ops/settings/save", { settings: draft })
        .then((r) => {
          setSaved(r);
    ...
    const saveToken = () => {
      ...
      setSaved({ ok: true, message: "运维令牌已保存。" });
    };
    ...
    return el("div", { className: "dshops-grid" },
      el(Card, {
        ...
        draft ? el(React.Fragment, null,
          ...
          el("div", { className: "dshops-row" },
            el("span", { className: "dshops-muted" }, t("settings.token")),
            el("input", { className: "dshops-input", type: "text", autoComplete: "off", value: token, onChange: (e) => setToken(e.target.value), placeholder: "粘贴本机生成的运维令牌" }),
            el(Btn, { onClick: saveToken }, t("settings.tokenSave")),
          ),
          el("div", { className: "dshops-muted" }, t("settings.tokenHint")),
  ...
  function ConsoleSection() {
    ...
    const auth = useApi("/dsh-ops/auth");
    useEffect(() => {
      if (typeof auth.data?.token === "string" && auth.data.token.length >= 32) saveAdminToken(auth.data.token);
    }, [auth.data?.token]);
    const changeLang = (l) => {
      currentLang = l;
    ...
    return el("div", { className: "dshops-root" },
      ...
      el("div", { className: "dshops-tabs" },
        el("span", { className: "dshops-muted" }, "dsh-ops-console v" + (auth.data?.opsVersion || "—")),
