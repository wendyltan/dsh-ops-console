...
// Fallback engine version for the version-check card. Set DSH_OPS_ENGINE to
// your pinned dsh version if it differs (e.g. you launch with npx @deepseek-ai/dsh@<x>).
const CURRENT_ENGINE = process.env.DSH_OPS_ENGINE ?? '0.1.0-rc.6'
  ...
  ctx.effect(() => {
    ...
    route('POST', '/dsh-ops/trust/add', async (req, res) => {
      ...
      if (current.includes(host)) {
        sendJson(res, 200, { ok: true, hosts: current, restartRequired: true, message: `「${host}」已在可信主机列表中。` })
        return
      }
      const w = writeTrustedHosts([...current, host])
      if (!w.ok) { sendJson(res, 500, { ok: false, error: w.error }); return }
      sendJson(res, 200, { ok: true, hosts: w.hosts, restartRequired: true, message: `已添加「${host}」，重启服务后生效。` })
    })
    ...
    route('POST', '/dsh-ops/trust/remove', async (req, res) => {
      ...
      const current = readTrustedHosts()
      if (!current.includes(host)) {
        sendJson(res, 200, { ok: true, hosts: current, message: `「${host}」不在可信主机列表中。` })
        return
      }
      const w = writeTrustedHosts(current.filter((h) => h !== host))
      if (!w.ok) { sendJson(res, 500, { ok: false, error: w.error }); return }
      sendJson(res, 200, { ok: true, hosts: w.hosts, restartRequired: true, message: `已移除「${host}」，重启服务后生效。` })
    })
