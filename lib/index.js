...
function registerOpsTools(ctx, disposers) {
  try {
    ...
    disposers.push(tools.register(def('ops_restart',
      'Safely restart dsh through the external guardian: preflight first, then restart, recover, or enter safe mode.',
      objectParameters(),
      resultSchema({ pid: { type: 'integer' } }),
      async () => {
        const r = await scheduleGuardian('restart')
        return r.ok
          ? { ok: true, pid: r.pid, scheduled: true, guardian: true }
          : { ok: false, error: r.error, guardian: false }
      })))
  } catch (error) {
