...
function scheduleGuardian(command) {
  if (!existsSync(GUARDIAN)) return scheduleRestart()
  const child = spawn(process.execPath, [GUARDIAN, command, '--json'], {
    detached: true, env: process.env, stdio: 'ignore',
  })
  child.unref()
  return { pid: process.pid, helperPid: child.pid, guardian: true, command }
}
  ...
  try {
    ...
    disposers.push(tools.register(def('ops_restart',
      ...
      async () => {
        ...
        return { ok: true, pid: r.pid, scheduled: true, guardian: r.guardian === true }
      })))
  } catch (error) {
