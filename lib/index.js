 ...
 * settings-page "运维控制台" (ops console): plain-language status, guided
 * diagnosis, a privacy-safe report, and optional Tailscale remote access.
 * All process/filesystem/network work uses Node builtins, so
...
const activity = [{
  ...
  kind: 'info',
  title: '运维控制台已打开',
  detail: '现在可以查看服务状态，或在遇到问题时开始检查。',
  technical: { opsVersion: OPS_VERSION },
}]
...
// ---------------------------------------------------------------- user-facing status and diagnosis

function recordActivity(kind, title, detail, technical = {}) {
  activity.unshift({ at: new Date().toISOString(), kind, title, detail, technical })
  if (activity.length > ACTIVITY_LIMIT) activity.length = ACTIVITY_LIMIT
}

async function userSummary() {
  const remote = await getTailscale()
  const remoteReady = remote.serveActive && remote.dnsName !== null
  return {
    ok: true,
    state: 'normal',
    title: '现在可以正常使用',
    detail: '这个页面已成功连接到 DeepSeek Harness。你不需要做任何事。',
    action: { id: 'diagnose', label: '检查问题' },
    remote: {
      state: remoteReady ? 'ready' : 'not-configured',
      title: remoteReady ? '可以在另一台设备上打开' : '尚未设置其他设备访问',
      detail: remoteReady
        ? '已准备好一个可从受信任设备打开的地址。'
        : '本机不受影响；如需在手机或另一台电脑查看，可在“在另一台设备上打开”中设置。',
    },
...
async function diagnose(symptom) {
  const remote = await getTailscale()
  const chosen = ['web', 'history', 'remote', 'slow', 'other'].includes(symptom) ? symptom : 'other'
  let title = '当前服务可以连接'
  let detail = '这个页面已成功连接到 DeepSeek Harness。'
  let impact = '你可以继续使用；如果问题仍在其他页面出现，请生成诊断报告。'
  let next = { id: 'report', label: '生成诊断报告' }

  if (chosen === 'remote') {
    if (remote.serveActive && remote.dnsName) {
      title = '其他设备访问已准备好'
      detail = '远程访问地址已配置。请确认另一台设备已加入你的受信任网络。'
      impact = '本机使用不受影响。'
      next = { id: 'remote', label: '查看访问方式' }
    } else {
      title = '其他设备暂时还不能打开'
      detail = '本机服务正常，但尚未设置可供其他设备使用的访问地址。'
      impact = '这不会影响在这台 Mac 上使用。'
      next = { id: 'remote', label: '设置其他设备访问' }
    }
  } else if (chosen === 'history') {
    title = '服务当前可以连接'
    detail = '历史记录是否完整需要在出现问题的页面再次确认。'
    impact = '若仍显示“历史加载失败”，请生成报告；报告不会包含会话正文。'
  } else if (chosen === 'slow') {
    title = '服务当前可以连接'
    detail = '此检查没有发现服务不可用。速度变慢可能与网络或正在处理的请求有关。'
    impact = '你可以继续使用；若持续变慢，请生成报告。'
  }

  recordActivity(chosen === 'remote' && !remote.serveActive ? 'attention' : 'check', title, detail, {
    symptom: chosen,
    remoteConfigured: remote.serveActive === true,
  })
  return { ok: true, symptom: chosen, title, detail, impact, next, checkedAt: new Date().toISOString() }
}
...
async function diagnosticReport() {
  ...
  return {
    ...
    summary: 'DeepSeek Harness 运维诊断报告（已自动脱敏）',
    findings: {
      serviceReachable: true,
      remoteAccessConfigured: remote.serveActive === true && remote.dnsName !== null,
      remoteAddress: remote.dnsName ? '[已配置]' : '[未配置]',
    },
    recentEvents: activity.slice(0, 10).map(({ at, kind, title, detail }) => ({ at, kind, title, detail })),
    technicalDetails: {
      opsVersion: OPS_VERSION,
      engine: CURRENT_ENGINE ?? '未识别',
      uptimeSeconds: Math.round(process.uptime()),
    },
    privacy: '本报告不包含密钥、令牌、提示词、会话正文、完整配置或远程地址。',
  }
}
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    route('GET', '/dsh-ops/summary', async (_req, res) => {
      sendJson(res, 200, await userSummary())
    })

    route('GET', '/dsh-ops/diagnose', async (req, res) => {
      const url = new URL(req.url, 'http://localhost')
      sendJson(res, 200, await diagnose(url.searchParams.get('symptom') ?? 'other'))
    })

    route('GET', '/dsh-ops/timeline', async (_req, res) => {
      sendJson(res, 200, { ok: true, events: activity.map(({ at, kind, title, detail }) => ({ at, kind, title, detail })) })
    })

    route('GET', '/dsh-ops/report', async (_req, res) => {
      sendJson(res, 200, await diagnosticReport())
    })
    ...
    route('POST', '/dsh-ops/tailscale/enable', async (_req, res) => {
      ...
      if (result.ok) recordActivity('change', '已设置其他设备访问', '现在可以从受信任设备打开 DeepSeek Harness。')
      sendJson(res, 200, result)
    })

    route('POST', '/dsh-ops/tailscale/disable', async (_req, res) => {
      const r = await runCommand(tailscaleCli(), ['serve', 'reset'], { timeoutMs: 10000 })
      if (r.code === 0) recordActivity('change', '已关闭其他设备访问', '这不会影响在本机使用 DeepSeek Harness。')
      sendJson(res, 200, {
        ok: r.code === 0,
