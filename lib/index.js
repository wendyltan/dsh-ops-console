...
async function runGuardian(command, { timeoutMs = 120000 } = {}) {
  ...
  try {
    ...
    return { ...parsed, available: true, command, exitCode: result.code }
  } catch {
...
function scheduleGuardian(command) {
  ...
  try {
    ...
    const opsStatus = async () => {
      ...
      return {
        ...
        url: BASE_URL,
        guardianAvailable: guardian.available === true,
        guardianMode: guardian.state?.mode ?? 'unknown',
        lastKnownGood: guardian.lastKnownGood === true,
        deploymentBackups: guardian.deploymentBackups?.length ?? 0,
      }
    }
    ...
    disposers.push(tools.register(def('ops_restart',
      ...
      async () => {
        const r = scheduleGuardian('restart')
        return r.ok
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    route('GET', '/dsh-ops/guardian', async (_req, res) => {
      sendJson(res, 200, await runGuardian('status', { timeoutMs: 15000 }))
    })
    ...
    route('POST', '/dsh-ops/preflight', async (_req, res) => {
      sendJson(res, 200, await runGuardian('preflight'))
    })
    ...
    route('POST', '/dsh-ops/recover', async (_req, res) => {
      if (!existsSync(GUARDIAN)) {
        sendJson(res, 503, { ok: false, error: `guardian missing: ${GUARDIAN}` })
        return
      }
      const r = scheduleGuardian('recover')
