...
function registerOpsTools(ctx, disposers) {
  try {
    ...
    disposers.push(tools.register(def('ops_logs',
      ...
      { tail: { type: 'integer', required: false, description: 'Number of log lines to return (10-500, default 100).' } },
    ...
    disposers.push(tools.register(def('ops_tailscale',
      ...
      {},
      resultSchema({ dnsName: { type: ['string', 'null'] }, serveActive: { type: 'boolean' }, trustedHosts: { type: 'array' } }),
      async () => getTailscale())))
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    route('POST', '/dsh-ops/settings', async (req, res) => {
