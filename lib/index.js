...
const GUARDIAN = process.env.DSH_GUARDIAN ?? join(DSH_HOME, 'guardian', 'guardian.mjs')
const DEPLOY_ROOT = process.env.DSH_OPS_DEPLOY_ROOT ?? join(DSH_HOME, 'deployments', 'dsh-ops-console')
const GUARDIAN_PROTOCOL_MIN = 1
const GUARDIAN_PROTOCOL_MAX = 2
const OPS_VERSION = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version
const ADMIN_TOKEN_FILE = join(DSH_HOME, 'ops-console', 'admin-token')

const BALANCE_URL = 'https://api.deepseek.com/user/balance'
const TAILSCALE_APP = '/Applications/Tailscale.app/Contents/MacOS/Tailscale'

/** Auto-detect the running dsh engine version; never invent a stale fallback. */
...
function adminToken() {
  const configured = process.env.DSH_OPS_ADMIN_TOKEN?.trim()
  if (configured?.length >= 32) return configured
  try {
    const existing = readFileSync(ADMIN_TOKEN_FILE, 'utf8').trim()
    if (existing.length >= 32) return existing
  } catch {}
  const token = randomBytes(32).toString('base64url')
  mkdirSync(dirname(ADMIN_TOKEN_FILE), { recursive: true, mode: 0o700 })
...
function isLocalHost(request) {
  try {
    const hostname = new URL(`http://${String(request.headers.host ?? '')}`).hostname
...
function hasAdminToken(request) {
  ...
  const supplied = Buffer.from(String(request.headers['x-dsh-ops-token'] ?? ''))
...
function authorizeMutation(request, response) {
  ...
  if (!hasAdminToken(request)) {
    sendJson(response, 401, { ok: false, error: '需要 dsh-ops 运维令牌', authRequired: true })
    return false
  }
  return true
}
...
async function runGuardian(command, { timeoutMs = 120000 } = {}) {
  if (!existsSync(GUARDIAN)) return { ok: false, available: false, error: `guardian missing: ${GUARDIAN}` }
  const result = await runCommand(process.execPath, [GUARDIAN, command, '--json'], { timeoutMs })
  try {
    const parsed = JSON.parse(result.stdout)
    const reportsProtocol = parsed.protocolVersion !== undefined
    const protocol = Number(parsed.protocolVersion ?? 0)
    const compatible = !reportsProtocol || (protocol >= GUARDIAN_PROTOCOL_MIN && protocol <= GUARDIAN_PROTOCOL_MAX)
    return { ...parsed, available: true, compatible, command, exitCode: result.code,
      ...(compatible ? {} : { ok: false, error: `unsupported guardian protocol: ${protocol}` }) }
  } catch {
    return { ok: false, available: true, command, exitCode: result.code, error: (result.stderr || result.stdout || 'guardian returned no JSON').slice(0, 1000) }
  }
}

function deploymentInfo() {
  const current = join(DEPLOY_ROOT, 'current')
  const backupDir = join(DEPLOY_ROOT, 'backups')
  const historyFile = join(DEPLOY_ROOT, 'history.json')
  let backups = []
  let history = []
  try { backups = readdirSync(backupDir).sort().reverse() } catch {}
  try { history = JSON.parse(readFileSync(historyFile, 'utf8')).slice(-20).reverse() } catch {}
  return { ok: true, root: DEPLOY_ROOT, current: existsSync(current) ? current : null, backups, history }
}

async function requireGuardianCapability(command) {
  if (!existsSync(GUARDIAN)) return { ok: false, guardian: false, error: `guardian missing: ${GUARDIAN}` }
  const capabilities = await runGuardian('capabilities', { timeoutMs: 15000 })
  if (!capabilities.ok || capabilities.compatible !== true || !capabilities.capabilities?.includes(command)) {
    return { ok: false, guardian: true, error: capabilities.error ?? `guardian capability unavailable: ${command}` }
  }
  return { ok: true, guardian: true }
}

async function runGuardedGuardian(command, options) {
  const supported = await requireGuardianCapability(command)
  return supported.ok ? await runGuardian(command, options) : supported
}

async function scheduleGuardian(command) {
  const supported = await requireGuardianCapability(command)
  if (!supported.ok) return supported
  const child = spawn(process.execPath, [GUARDIAN, command, '--json'], {
    detached: true, env: process.env, stdio: 'ignore',
  })
  child.unref()
  return { ok: true, pid: process.pid, helperPid: child.pid, guardian: true, command }
}
...
function isBareAuthority(entry) {
  ...
  if (entry.includes(':')) {
    ...
    if (rest.length !== 1 || !/^\d{1,5}$/.test(rest[0]) || Number(rest[0]) > 65535) return false
 ...
 * check balance, read logs, and inspect remote access. Tool
...
function registerOpsTools(ctx, disposers) {
  try {
    ...
    const opsStatus = async () => {
      const guardian = await runGuardian('status', { timeoutMs: 15000 })
      return {
        ok: true,
        up: true,
        host: HOST,
        ...
        url: BASE_URL,
        guardianAvailable: guardian.available === true && guardian.compatible !== false,
        guardianMode: guardian.state?.mode ?? 'unknown',
        lastKnownGood: guardian.lastKnownGood === true,
        deploymentBackups: deploymentInfo().backups.length,
      }
    }
    ...
    disposers.push(tools.register(def('ops_preflight',
      'Run the external DSH guardian preflight and isolated smoke boot without stopping the live service.',
      objectParameters(),
      resultSchema({ stage: { type: 'string' }, issues: { type: 'array', items: { type: 'string' } } }),
      async () => runGuardedGuardian('preflight'))))
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    const route = (method, path, handler) => {
      disposers.push(ctx.webServer.register({
        ...
        handler: async (request, response) => {
          ...
          if (method === 'POST' && !authorizeMutation(request, response)) return
          try {
    ...
    route('GET', '/dsh-ops/auth', async (req, res) => {
      sendJson(res, 200, {
        ok: true,
        opsVersion: OPS_VERSION,
        tokenRequired: true,
        // A Tailnet request is proxied from loopback, so Host—not socket address—
        // is the boundary for bootstrap token disclosure.
        token: isLocalHost(req) ? adminToken() : undefined,
      })
    })
    ...
    route('GET', '/dsh-ops/guardian', async (_req, res) => {
      const guardian = await runGuardian('status', { timeoutMs: 15000 })
      const deployments = deploymentInfo()
      sendJson(res, 200, { ...guardian, deploymentBackups: deployments.backups })
    })

    route('GET', '/dsh-ops/deployments', async (_req, res) => {
      sendJson(res, 200, deploymentInfo())
    })

    route('POST', '/dsh-ops/preflight', async (_req, res) => {
      const result = await runGuardedGuardian('preflight')
      sendJson(res, result.ok ? 200 : 503, result)
    })

    route('POST', '/dsh-ops/restart', async (_req, res) => {
      const r = await scheduleGuardian('restart')
      if (!r.ok) {
        sendJson(res, 503, r)
        return
      }
      sendJson(res, 200, { ok: true, pid: r.pid, guardian: r.guardian === true, msgKey: 'restart.triggered', message: '已触发安全重启：先预检，再重启；失败时自动恢复或进入安全模式。' })
    })

    route('POST', '/dsh-ops/recover', async (_req, res) => {
      const r = await scheduleGuardian('recover')
      if (!r.ok) {
        sendJson(res, 503, r)
        return
      }
      sendJson(res, 200, { ok: true, pid: r.pid, guardian: r.guardian === true, message: '已安排恢复黄金版本并重启。' })
    })
